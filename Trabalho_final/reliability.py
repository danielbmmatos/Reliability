...
#------------------------------------------------------------------------------
""" 
This Python class is developed to solve reliability problems with FORM and 
Monte Carlo Analysis. The codes you will find here are adaptations of 
Prof. Phd. Herbert Martins Gomes MATLAB codes.
...
#------------------------------------------------------------------------------    
    def FORM(FEL,mean,std,dist,rho):
        ...
        S  = np.diag(std)
        X0 = mean
        Seq,meq = reliability.equiv(mean,S,X0,dist)
        Rz      = reliability.nataf(dist,rho,X0,std)
        ...
        while crit > tol:    #Main loop
            ...
            #grad[0] = 3*x[0]**2+2*x[0]*x[1]

            #grad[1] = x[0]**2+3*x[1]**2
        ...
        with np.printoptions(precision = 3):
            ...
            print('---------------------------------------------','\n',
                      '                     FORM          ','\n',
                      '--------------------------------------------','\n',
                      'Iteração:{0:5d}'.format(k),'\n',
                      'Alfa ={}'.format(alfa),'\n',
                      'Beta = {0:5.6f}'.format(beta),'\n',
                      'Pfalha = {0:5.3e}'.format(Pf),'\n',
                      'Critério = {0:5.3e}'.format(crit),'\n',
                      'Chamadas da FEL = {0:5.1e}'.format(nfunc),'\n'
                      'MPP ={}'.format(zn))  
            #if k>20:
                #break
        return beta
...
#------------------------------------------------------------------------------    
    ...
    def MONTECARLO(FEL,mean,std,dist,rho,n,nl):
        ...
        print('---------------------------------------------','\n',
                ...
                #if CVPf[k-1]<=0.01: # criteria to minimize simulations
                    #break
